...
export function parse(buffer: ArrayBuffer): Song {
  ...
  const { measureHeaders, repeatGroups } = parseMeasureHeaders(
    stream,
    measureCount,
    tripletFeel
  );
...
*/
export function parseMeasureHeaders(
  stream: GtpStream,
  measureCount: number,
  ...
  for (let index = 1; index < measureCount + 1; index++) {
    const measureHeader = parseMeasureHeader(stream, measureHeaders, index, {
...
export function parseMeasureHeader(
  stream: GtpStream,
...
export function parseMeasures(
  ...
  for (const measureHeader of measureHeaders) {
    ...
    for (const track of tracks) {
      ...
      const voice = parseVoice(stream, track, measure);
...
export function parseVoice(
  ...
  for (let i = 0; i < beatCount; i++) {
    const beat = parseBeat(stream, i, startTime, voice, track, measure);
...
export function parseBeat(
  ...
  const beat =
    voice.beats.reverse().find((beat) => {
      return beat.start === start;
    }) ||
    ({
      index,
      flag,
  ...
  if (flag & BeatFlag.ChordDiagram) {
    beat.effect.chord = parseChord(stream, track.strings.length);
  }
  ...
  if (flag & BeatFlag.Effect) {
    ...
    beat.effect = parseBeatEffect(stream);
    beat.effect.chord = chord;
  }

  if (flag & BeatFlag.MixTable) {
    beat.effect.mixTableChange = parseMixTableChange(stream, measure);
  }

  beat.notes = parseNotes(stream, voice, track, beat);
...
export function parseChord(stream: GtpStream, stringCount: number): Chord {
  const isNewFormat = stream.readBool();
  if (isNewFormat) {
    return parseNewFormatChord(stream, stringCount);
  } else {
    return parseOldFormatChord(stream, stringCount);
  }
}

export function parseOldFormatChord(
  stream: GtpStream,
  stringCount: number
): Chord {
...
export function parseNewFormatChord(
  stream: GtpStream,
  stringCount: number
): Chord {
...
export function parseBeatEffect(stream: GtpStream): BeatEffect {
  ...
  if (flag & BeatEffectFlag.TremoloBarOrSlapEffect) {
    ...
    if (beatEffect.slapEffect === SlapEffect.None) {
      beatEffect.tremoloBar = parseTremoloBarEffect(stream);
    } else {
      stream.readInt32();
    }
  }
  ...
  if (flag & BeatEffectFlag.BeatStrokeDirection) {
    beatEffect.stroke = parseBeatStroke(stream);
  }
  ...
  if (flag & BeatEffectFlag.ArtificialHarmonic) {
    beatEffect.harmonic = { type: 2, pitch: { just: 0 }, octave: Octave.None };
  }

  return beatEffect;
}

export function parseTremoloBarEffect(stream: GtpStream): BendEffect {
...
export function parseBeatStroke(stream: GtpStream): BeatStroke {
  ...
  if (up > 0) {
    return {
      ...
      value: toStrokeValue(up),
    };
  } else {
    return {
      direction: BeatStrokeDirection.Down,
      value: toStrokeValue(down),
    };
  }
}

export function toStrokeValue(value: number) {
  switch (value) {
    case 1:
...
export function parseMixTableChange(stream: GtpStream, measure: Measure) {
  ...
  const balance = stream.readByte();
  if (balance >= 0) {
    mixTableChange.balance = { value: balance, duration: 0, allTracks: false };
  }
  ...
  const tremolo = stream.readByte();
  if (tremolo >= 0) {
    mixTableChange.tremolo = { value: tremolo, duration: 0, allTracks: false };
  }
...
export function parseNotes(
  ...
  for (const string of track.strings) {
    if (flag & (1 << (7 - string.index))) {
      const note = parseNote(stream, track, beat, string);
      notes.push(note);
    }
  }
...
export function parseNote(
  ...
  if (flag & NoteFlag.NoteEffectPresence) {
    note.effect = parseNoteEffect(stream);
  }
...
export function parseNoteEffect(stream: GtpStream) {
  const flag = stream.readByte();
  ...
  if (flag & NoteEffectFlag.Bend) {
    effect.bend = parseBend(stream);
  }

  if (flag & NoteEffectFlag.Grace) {
    effect.grace = parseGrace(stream);
  }

  if (flag & NoteEffectFlag.Slides) {
    effect.slides = parseSlides(stream);
  }

  return effect;
}

export function parseBend(stream: GtpStream): BendEffect {
  const type = stream.readByte();
  const value = stream.readInt32(true);
  const points = [];
...
export function parseGrace(stream: GtpStream): GraceEffect {
...
export function parseSlides(stream: GtpStream) {
  return [SlideType.ShiftSlideTo];
}

export function parseColor(stream: GtpStream): Color {
  const color = {
    r: stream.readByte(),
    g: stream.readByte(),
    b: stream.readByte(),
